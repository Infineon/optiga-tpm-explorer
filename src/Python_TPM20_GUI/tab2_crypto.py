...
class Tab_RSA(wx.Panel):
    def __init__(self, parent):
        ...
        # create tooltips
        createprimary.SetToolTip(
            wx.ToolTip(
                "TPM2_CreatePrimary: Create a Primary Key Pair under the Owner Authorisation. Handle = 0x81000004"
            )
        )
        createkeypairbutton.SetToolTip(
            wx.ToolTip(
                "TPM2_Create: Create a RSA Key Pair under the Primary key (that you should have generated already). Handle = 0x81000005"
            )
        )
        encbutton.SetToolTip(
            wx.ToolTip("TPM2_RSAEncrypt: Encrypt data input using the RSA public key")
        )
        ...
        opensslverifybutton.SetToolTip(
            wx.ToolTip("Using OpenSSL, verify the signature with the public key")
        )
        tpmverifybutton.SetToolTip(
            wx.ToolTip(
                "TPM2_VerifySignature: Using the TPM, verify the signature with the public key"
            )
        )
        clearbutton.SetToolTip(wx.ToolTip("Clear all textboxes."))
    ...
    def OnCreatePrimary(self):
        ...
        if evict_status == True:
            self.command_display.AppendText(
                f"tpm2_evictcontrol -C o -P {owner_auth} -c 0x81000004 \n"
            )
        self.command_display.AppendText(
            "tpm2_createprimary -C o -P " + owner_auth + " -g sha256 -G rsa -c RSAprimary.ctx\n"
        )
        self.command_display.AppendText(
            "tpm2_evictcontrol -C o -c RSAprimary.ctx -P " + owner_auth + " 0x81000004\n"
        )
        self.command_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
    ...
    def OnCreateKeyPair(self):
        ...
        if evict_status == True:
            self.command_display.AppendText(
                f"tpm2_evictcontrol -C o -P {owner_auth} -c 0x81000005 \n"
            )
        self.command_display.AppendText(
        ...
        self.command_display.AppendText(
            "tpm2_evictcontrol -a o -c RSAkeycontext.ctx -p 0x81000005 -P " + owner_auth + "\n"
        )
        self.command_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
...
class Tab_ECC(wx.Panel):
    def __init__(self, parent):
        ...
        createprimary.SetToolTip(
            wx.ToolTip(
                "TPM2_CreatePrimary: Create a Primary Key Pair under the Owner Authorisation. Handle = 0x81000006"
            )
        )
        createkeypairbutton.SetToolTip(
            wx.ToolTip(
                "TPM2_Create: Create an ECC Key Pair under the Primary key (that you should have generated already). Handle = 0x81000007"
            )
        )
        signbutton.SetToolTip(wx.ToolTip("TPM2_Sign: Sign the data input with the private key"))
        opensslverifybutton.SetToolTip(
    ...
    def OnCreatePrimary(self):
        ...
        self.Update()
        output_message = exec_cmd.execTpmToolsAndCheck(
            ["tpm2_evictcontrol", "-C", "o", "-c", "ECCprimary.ctx", "-P", owner_auth, "0x81000006"]
        )
        self.command_display.AppendText(str(output_message) + "\n")
        if evict_status == True:
            self.command_display.AppendText(
                f"tpm2_evictcontrol -C o -P {owner_auth} -c 0x81000006 \n"
            )
        self.command_display.AppendText(
            "tpm2_createprimary -C o -P " + owner_auth + " -g sha256 -G 0x0023 -c ECCprimary.ctx\n"
        )
        self.command_display.AppendText(
            "tpm2_evictcontrol -C o -c ECCprimary.ctx -p 0x81000006 -P " + owner_auth + "\n"
        )
        self.command_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
    ...
    def OnCreateKeyPair(self):
        ...
        if evict_status == True:
            self.command_display.AppendText(
                f"tpm2_evictcontrol -C o -P {owner_auth} -c 0x81000007 \n"
            )
        self.command_display.AppendText(
        ...
        self.command_display.AppendText(
            "tpm2_evictcontrol -C o -c ECCkeycontext.ctx -P " + owner_auth + " 0x81000007\n"
        )
        self.command_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
