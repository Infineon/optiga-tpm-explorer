...
class RestartApp(wx.Dialog):
    def __init__(self, parent, title):
        ...
        text_input_blurb = wx.StaticText(
            self,
            -1,
            "The TPM Explorer Application will close now.\n\
...
class CredentialDlg(wx.Dialog):
    def __init__(self, parent, title):
        ...
        mainsizer.Add(
            wx.StaticText(
                self,
                -1,
                "You may leave some inputs blank, to set only a specific authorisation. All spaces will be stripped, if any",
            ),
            1,
            wx.ALL,
            5,
    ...
    def OnClearOwnerAuth(self, evt):
        ...
        if exec_cmd.ownerAuth != "":
            ...
            self.parent.text_display.AppendText(
                "'tpm2_changeauth -c o -p " + exec_cmd.ownerAuth + "' executed \n"
            )
            self.parent.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
            if "ERROR" not in command_output:
                exec_cmd.ownerAuth = ""
                exec_cmd.save_partial_auth(
                    ownerAuth="", endorseAuth=None, lockoutAuth=None, nvAuth=None
                )
    ...
    def OnClearEndorseAuth(self, evt):
        ...
        if exec_cmd.endorseAuth != "":
            ...
            self.parent.text_display.AppendText(str(command_output))
            self.parent.text_display.AppendText(
                "'tpm2_changeauth -c e -p " + exec_cmd.endorseAuth + "' executed \n"
            )
            self.parent.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
            if "ERROR" not in command_output:
                exec_cmd.endorseAuth = ""
                exec_cmd.save_partial_auth(
                    ownerAuth=None, endorseAuth="", lockoutAuth=None, nvAuth=None
                )
    ...
    def OnClearLockoutAuth(self, evt):
        ...
        if exec_cmd.lockoutAuth != "":
            ...
            self.parent.text_display.AppendText(str(command_output))
            self.parent.text_display.AppendText(
                "'tpm2_changeauth -c l -p " + exec_cmd.lockoutAuth + "' executed \n"
            )
            self.parent.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
            if "ERROR" not in command_output:
                exec_cmd.lockoutAuth = ""
                exec_cmd.save_partial_auth(
                    ownerAuth=None, endorseAuth=None, lockoutAuth="", nvAuth=None
                )
...
class DictAttackDlg(wx.Dialog):
    def __init__(self, parent, title):
        ...
        maxAuthFail_input_blurb = wx.StaticText(
            self,
            -1,
            "TPM will be in Lockout Mode when failed attempts reaches this number\n (default/max = 0x20, or 32): ",
        )
        self.lockoutInterval_input = wx.TextCtrl(self, -1)
        lockoutInterval_input_blurb = wx.StaticText(
            self,
            -1,
            "No. of seconds it takes to decrement the failed attempts counter\n (2^32-1 prevents exit from Lockout Mode)(Default is 0x1C20, or 7200): ",
        )
        self.lockoutRecovery_input = wx.TextCtrl(self, -1)
        lockoutRecovery_input_blurb = wx.StaticText(
            self,
            -1,
            "Delay in seconds between attempts to use LockoutAuth\n (0 indicates a system reboot is required instead)(Default is 0x15180, or 86400): ",
        )
        button_ok = wx.Button(self, -1, "OK")
...
class ClearWarningDlg(wx.MessageDialog):
    def __init__(self, parent, title):
        wx.MessageDialog.__init__(
            self, parent, message="", caption=title, style=wx.OK | wx.ICON_WARNING | wx.CANCEL
        )
    ...
    def __init__(self, parent, title):
        ...
        mainsizer.Add(
            wx.StaticText(
                self,
                -1,
                "ALL TPM Engine features require ownerAuth to be set to NULL.\n\
...
class AwsRegionWarning(wx.MessageDialog):
    def __init__(self, parent, title):
        ...
        self.SetMessage(
            "WARNING, as you have set the AWS region to a NON us-east-1 region, when attempting to publish to the AWS Cloud, the application will crash.\n\n\
To fix this problem:\n\
