...
class Tab_Setup(wx.Panel):
    def __init__(self, parent):
        ...
        button_start.SetToolTip(
            wx.ToolTip(
                "TPM2_Startup: Required to be run after pressing the hardware reset. Not doing so will result in the TPM being unresponsive."
            )
        )
        button_takeown.SetToolTip(
            wx.ToolTip("TPM2_ChangeAuth: Set Hierarchy Authorisation Passwords")
        )
        button_clear.SetToolTip(
            wx.ToolTip(
                "TPM2_Clear: Using Platform Authorisation to reset all TPM settings and contexts."
            )
        )
        button_dictAtk.SetToolTip(
            wx.ToolTip(
                "TPM2_DictionaryLockout: Set values dictating how the TPM should handle failed authorisation attempts."
            )
        )
        button_disablelock.SetToolTip(
            wx.ToolTip("TPM2_ClearControl: Disable the use of TPM2_Clear")
        )
        button_enablelock.SetToolTip(wx.ToolTip("TPM2_ClearControl: Enable the use of TPM2_Clear"))
        button_getCapVar.SetToolTip(
            wx.ToolTip(
                "TPM2_GetCap: Get TPM variables, such as whether hierarchies are enabled, or dictionary attack settings."
            )
        )
        button_getCapFix.SetToolTip(
            wx.ToolTip(
                "TPM2_GetCap: Get TPM constants, such as Manufacturer, Vendor, Firmware values."
            )
        )
    ...
    def OnChangeAuth(self, evt):
        ...
        if exec_cmd.ownerAuth != "":
            ...
            self.text_display.AppendText(str(command_output))
            self.text_display.AppendText(
                "'tpm2_changeauth -c owner " + exec_cmd.ownerAuth + "' executed \n"
            )
            if "ERROR" not in command_output:  # Check for errors in the command output
                exec_cmd.save_partial_auth(
                    ownerAuth=exec_cmd.ownerAuth, endorseAuth=None, lockoutAuth=None, nvAuth=None
                )
        if exec_cmd.endorseAuth != "":
            ...
            self.text_display.AppendText(str(command_output))
            self.text_display.AppendText(
                "'tpm2_changeauth -c endorsement " + exec_cmd.endorseAuth + "' executed \n"
            )
            if "ERROR" not in command_output:  # Check for errors in the command output
                exec_cmd.save_partial_auth(
                    ownerAuth=None, endorseAuth=exec_cmd.endorseAuth, lockoutAuth=None, nvAuth=None
                )
        if exec_cmd.lockoutAuth != "":
            ...
            if "ERROR" not in command_output:  # Check for errors in the command output
                exec_cmd.save_partial_auth(
                    ownerAuth=None, endorseAuth=None, lockoutAuth=exec_cmd.lockoutAuth, nvAuth=None
                )
    ...
    def OnEnableLock(self, evt):
        ...
        self.text_display.AppendText(
            "'tpm2_clearcontrol -C l s -P " + lockout_auth + "' executed \n"
        )
        self.text_display.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
...
class Tab_PCR(wx.Panel):
    def __init__(self, parent):
        ...
        self.pcr_bank_choice = wx.ComboBox(
            self, -1, "Pick the PCR Index", choices=pcr_index_list, style=wx.CB_READONLY
        )
        ...
        self.bottom_txt_display.SetFont(
            wx.Font(12, wx.FONTFAMILY_TELETYPE, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL)
        )
        clearimage = wx.Image("../images/clear.png", wx.BITMAP_TYPE_PNG).ConvertToBitmap()
        clearbutton = wx.BitmapButton(self, -1, clearimage)
        ...
        # Set tooltips
        button_pcrlistall.SetToolTip(
            wx.ToolTip(
                "TPM2_PCRList: List the value of a specific Platform Configuration Register (PCR) bank."
            )
        )
        button_pcrlist.SetToolTip(
            wx.ToolTip(
                "TPM2_PCRList: List the value of a specific Platform Configuration Register (PCR) Index."
            )
        )
        button_pcrextend.SetToolTip(
            wx.ToolTip(
                "TPM2_PCRExtend: New PCR Value is the hash of the concatenation of the current PCR bank value and the data input."
            )
        )
        button_pcrevent.SetToolTip(
            wx.ToolTip(
                "TPM2_PCREvent: Similar to Extend, but the data input is now hashed before being concatenated with the PCR bank value."
            )
        )
        clearbutton.SetToolTip(wx.ToolTip("Clear all textboxes."))
    ...
    def OnSHACheckboxChanged(self, event):
        ...
        if selected_sha == "SHA-256":
            ...
            self.bottom_txt_display.AppendText(
                "'tpm2_pcrallocate sha1:none+sha256:all+sha384:none' executed\n"
            )
        elif selected_sha == "SHA-384":
            command_output = exec_cmd.execTpmToolsAndCheck(
                ["tpm2_pcrallocate", "sha1:none+sha256:none+sha384:all"]
            )
            self.bottom_txt_display.AppendText(
                "'tpm2_pcrallocate sha1:none+sha256:none+sha384:all' executed\n"
            )
        else:  # default case for SHA-1
            command_output = exec_cmd.execTpmToolsAndCheck(
                ["tpm2_pcrallocate", "sha1:all+sha256:none+sha384:none"]
            )
            self.bottom_txt_display.AppendText(
                "'tpm2_pcrallocate sha1:all+sha256:none+sha384:none' executed\n"
            )
        ...
        if result == wx.ID_YES:
            command_output = exec_cmd.execTpmToolsAndCheck(["tpm2_startup", "--clear"])
            self.bottom_txt_display.AppendText("'tpm2_startup --clear' executed\n")
        else:
            self.sha_listchoice.SetSelection(
                0
            )  # Reverts to "SHA-1" (or whatever default you prefer)
    ...
    def OnPCRListAll(self, evt):
        ...
        hash_map = {"SHA-1": "sha1", "SHA-256": "sha256", "SHA-384": "sha384"}
...
class Tab_NVM(wx.Panel):
    def __init__(self, parent):
        ...
        self.filename_input = wx.TextCtrl(
            self,
            -1,
            value="ifx_ecc_cert.crt",
        ...
        self.right_txt_display.SetFont(
        ...
        # Set tooltips
        button_nvdefine.SetToolTip(
            wx.ToolTip(
                "TPM2_NVDefine: Cause the TPM to reserve space and associate that space with the NV index and attributes."
            )
        )
        button_nvwrite.SetToolTip(
            wx.ToolTip(
                "TPM2_NVWrite: Write to an area in NVM that is defined in TPM2_NVList. (if allowed by attributes)"
            )
        )
        button_nvrelease.SetToolTip(
            wx.ToolTip(
                "TPM2_NVRelease: Removes an index from the TPM and release the associated space"
            )
        )
        button_nvread.SetToolTip(
            wx.ToolTip(
                "TPM2_NVRead: Read from an area in NVM that is defined in TPM2_NVList. (if allowed by attributes)"
            )
        )
        button_nvrelock.SetToolTip(
            wx.ToolTip(
                "TPM2_NVReadLock: If READ_STCLEAR is an attribute set in the NV index, this will prevent further reads of the index, until the next TPM2_Startup."
            )
        )
        button_nvlist.SetToolTip(wx.ToolTip("TPM2_NVList: List all defined areas in the TPM"))
        button_reset_attr.SetToolTip(
            wx.ToolTip("Reset attributes to OwnerRead, OwnerWrite and Read_STClear.")
        )
        clearbutton.SetToolTip(wx.ToolTip("Clear all textboxes."))
        ...
        self.nvm_attr.SetCheckedStrings(
            ["ownerwrite", "ownerread", "authread", "authwrite", "read_stclear"]
        )
        self.SetSizer(mainsizer)
        mainsizer.Fit(self)
        self.Show(True)
    ...
    def OnClickFileName(self, evt):
        ...
        openFileDialog = wx.FileDialog(
            ...
            "",
            "",
            "All|*.bin;*.crt;*.der|Binary|*.bin|Certificate|*.crt;*.der",
    ...
    def OnResetAttr(self, evt):
        self.nvm_attr.SetCheckedStrings(
            ["authread", "authwrite", "ownerwrite", "ownerread", "read_stclear"]
        )
    ...
    def OnNVDefine(self, evt):
        ...
        filename = "tpm_auth.json"
        default_values = {"ownerAuth": "", "endorseAuth": "", "lockoutAuth": "", "nvAuth": ""}
...
class Tab_Handles(wx.Panel):
    def __init__(self, parent):
        ...
        # Set tooltips
        button_listpersistent.SetToolTip(
            wx.ToolTip("List what handles exists in the persistent store.")
        )
        button_readpersistent.SetToolTip(
            wx.ToolTip("List what context exists in the specific persistent handle.")
        )
        button_evict_persistent.SetToolTip(
            wx.ToolTip("TPM2_EvictControl: Remove specified handle from the persistent store.")
        )
    ...
    def OnEvict(self, evt):
        ...
        command_output = exec_cmd.execTpmToolsAndCheck(
            [
                ...
                specific_handle,
                "-P",
                exec_cmd.get_auth_from_config("owner"),
            ]
        )
        self.txt_display.AppendText(str(command_output))
        self.txt_display.AppendText(
