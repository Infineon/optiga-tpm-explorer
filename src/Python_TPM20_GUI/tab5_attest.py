...
class Tab5Frame(wx.Frame):
    def __init__(self, parent, title):
        ...
        button_verify_quote_tpm = wx.Button(
            self, -1, "Verify Quote (By TPM2-Tools)", size=(350, 48)
        )
        ...
        # Set tooltips
        button_gen_ak.SetToolTip(
            wx.ToolTip(
                "TPM2_GenEK/GenAK: Generate an Endorsement Key Pair, and then the Attestation Key Pair from it."
            )
        )
        button_gen_quote.SetToolTip(
            wx.ToolTip(
                "TPM2_Quote: Generate a quote of a PCR index, signed by the TPM's Attestation Key."
            )
        )
        #         button_verify_quote_ssl.SetToolTip(wx.ToolTip("Verify the quote using OpenSSL and the Attestation Public Key."))
        button_verify_quote_tpm.SetToolTip(
            wx.ToolTip("TPM2_CheckQuote: Verify the quote using TPM functions.")
        )
    ...
    def OnGenEK(self, evt):
        ...
        self.command_out.AppendText(
            ...
            + " -w "
            + owner_auth
            + " -c 0x81010001 -G rsa -u ek.pub' executed \n"
        )
        self.command_out.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
    ...
    def OnGenAK(self):
        ...
        command_output = exec_cmd.execTpmToolsAndCheck(
            ["tpm2_evictcontrol", "-C", "o", "-P", owner_auth, "-c", "ak.ctx", specific_handle]
        )
        ...
        if self.akevict == True:
            self.command_out.AppendText(
                f"tpm2_evictcontrol -C o -P {owner_auth} -c {specific_handle} \n"
            )
        self.command_out.AppendText(
            "'tpm2_createak -P "
            + endorse_auth
            + " -C 0x81010001 -G rsa -u ak_pub.bin -n ak.name -c ak.ctx' executed \n"
        )
        self.command_out.AppendText(
            "'tpm2_evictcontrol -C o -P "
            + owner_auth
            + " -c ak.ctx "
            + specific_handle
            + " executed \n"
    ...
    def OnGenQuote(self):
        ...
        if input_nonce == "":
            ...
            self.command_out.AppendText(
                "Error! Nonce must be of even length as it will be converted to a byte array.\n"
            )
            return
        ...
        self.command_out.AppendText(
            ...
            + " -m tpmquote.data -q "
            + input_nonce
            + " -f plain -s quote_sign_data' executed \n"
        )
        self.command_out.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
        command_output = exec_cmd.execTpmToolsAndCheck(
            [
        ...
        self.command_out.AppendText("\n")
        self.command_out.AppendText(
            "'tpm2_readpublic -c " + specific_handle + " -f pem -o ak_pub.pem' executed \n"
        )
        self.command_out.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
    ...
    def OnVerifyQTPM(self, evt):
        ...
        self.command_out.AppendText(str(command_output))
        self.command_out.AppendText(
            "'tpm2_checkquote -c ak_pub.pem -m tpmquote.data -s quote_sign_data -G sha256 -q "
            + input_nonce
            + "' executed \n"
        )
    ...
    def OnEvict(self, evt):
        ...
        self.command_out.AppendText(str(command_output))
        self.command_out.AppendText(
            "'tpm2_evictcontrol -C o -c " + specific_handle + " -P " + owner_auth + "' executed \n"
        )
        self.command_out.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
