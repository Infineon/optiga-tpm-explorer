...
class Tab4Frame(wx.Frame):
    def __init__(self, parent, title):
        ...
        button_create_policy = wx.Button(
            self, -1, "Generate Policy from selected PCR", size=(-1, 48)
        )
        ...
        # Set tooltips
        button_create_policy.SetToolTip(
            wx.ToolTip(
                "TPM2_PolicyPCR: Create a policy based on a PCR index, using a trial policy session."
            )
        )
        button_create_primary.SetToolTip(
            wx.ToolTip(
                "TPM2_CreatePrimary: Create a Primary Key Pair under the Owner Authorisation. Handle: 0x81000001"
            )
        )
        button_seal_data.SetToolTip(
            wx.ToolTip("TPM2_Create: Create a file containing data input to be sealed into the TPM")
        )
        button_satisfy_unseal.SetToolTip(
            wx.ToolTip(
                "TPM2_PolicyPCR: Using a non-trial policy session, check if the PCR index satisfies the policy. TPM2_Unseal: Returns data in a loaded sealed TPM object, provided the policy is satisfied."
            )
        )
    ...
    def OnNewPolicy(self):
        ...
        self.command_out.AppendText(
            "'tpm2_pcrread sha256:" + pcr_choice + " -o pcr.dat' executed \n"
        )
        command_output = exec_cmd.execTpmToolsAndCheck(
            [
        ...
        self.command_out.AppendText(str(command_output))
        self.command_out.AppendText(
            "'tpm2_policypcr -S session.dat -l sha256:"
            + pcr_choice
            + " -f pcr.dat -L policy.dat' executed \n"
        )
        command_output = exec_cmd.execTpmToolsAndCheck(
            [
                "tpm2_flushcontext",
    ...
    def OnNewPrimary(self):
        ...
        self.command_out.AppendText(
            ...
            + owner_auth
            + " -g sha256 -G ecc -c primary.ctx' executed \n"
        )
        self.command_out.AppendText(
            "'tpm2_evictcontrol -a o -c primary.ctx -P " + owner_auth + " 0x81000001' executed \n"
        )
        self.command_out.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
