...
def ServerProcess(server_log):
    ...
    server_proc = exec_cmd.createProcess(
        "lxterminal --command='openssl s_server -cert CAsigned_rsa_cert.crt -accept 4433 -keyform engine -engine tpm2tss -key rsa_server.tss'",
        server_log,
    )
...
def ClientProcess(client_log):
    ...
    if client_proc is not None:
        try:
            ...
            client_proc = None
    client_proc = exec_cmd.createProcess(
        "lxterminal --command='openssl s_client -connect localhost:4433 -tls1_2 -CAfile CA_rsa_cert.pem'",
        client_log,
    )
...
class Tab_RSA_CS(wx.Panel):
    def __init__(self, parent):
        ...
        # Set tooltips
        button_gen_ca.SetToolTip(
            wx.ToolTip(
                "Generate Key Pair and Self-Signed Certificate for the Certificate Authority (CA)."
            )
        )
        button_gen_keypair.SetToolTip(wx.ToolTip("Generate Key Pair for the Server."))
        button_gen_csr.SetToolTip(
            wx.ToolTip(
                "Generate Certificate Signing Request (CSR) for the CA, from the Server's private key."
            )
        )
        button_gen_cert.SetToolTip(
            wx.ToolTip("Generate Server Certificate from the CSR and the CA's private key.")
        )
    ...
    def Destroy(self):
        global \
            server_proc, \
            client_proc, \
            RSA_Server_thread_active_flag, \
            RSA_Client_thread_active_flag
        if server_proc is not None:
            RSA_Server_thread_active_flag = 0
    ...
    def OnGenKeyPair(self):
        ...
        if "0x8100000A" in output:
            ...
            self.text_server.AppendText(
                f"'tpm2_evictcontrol -C o -c 0x8100000A -P {owner_auth}' executed \n"
            )
        ...
        self.text_server.AppendText(
            f"'tpm2_createprimary -C o -P {owner_auth} -g sha256 -G rsa -c RSAprimary.ctx' executed \n"
        )

        output2 = exec_cmd.execCLI(
            ["tpm2_evictcontrol", "-C", "o", "-c", "RSAprimary.ctx", "-P", owner_auth, "0x8100000A"]
        )
        self.text_server.AppendText(
            f"'tpm2_evictcontrol -C o -c RSAprimary.ctx -P {owner_auth} 0x8100000A' executed \n"
        )
    ...
    def OnStartClient(self, evt):
        ...
        if client_proc is not None:
            ...
            if server_proc is not None:
                ...
                client_thread.start()
                wx.CallAfter(
                    Publisher.sendMessage, "Client_Text", msg="\n\n" + openssl_cmd + "\n\n"
                )
            else:
...
class Tab_ECC_CS(wx.Panel):
    ...
    def OnGenCA(self):
        ...
        command_output = exec_cmd.execCLI(
            [
                ...
                "CA_ecc_cert.pem",
                "-subj",
                "/C=SG/ST=Singapore/L=Singapore/O=Infineon Technologies/OU=CSS/CN=TPMEvalKitCA",
            ]
        )
        self.text_server.AppendText(str(command_output))
        self.text_server.AppendText(
    ...
    def OnGenKeyPair(self):
        ...
        if "0x8100000B" in output:
            ...
            self.text_server.AppendText(
                f"'tpm2_evictcontrol -C o -c 0x8100000B -P {owner_auth}' executed \n"
            )
        ...
        self.text_server.AppendText(
            f"'tpm2_createprimary -C o -P {owner_auth} -g sha256 -G ecc -c ECprimary.ctx' executed \n"
        )

        output2 = exec_cmd.execCLI(
            ["tpm2_evictcontrol", "-C", "o", "-c", "ECprimary.ctx", "-P", owner_auth, "0x8100000B"]
        )

        self.text_server.AppendText(
            f"'tpm2_evictcontrol -C o -c ECprimary.ctx -P {owner_auth} 0x8100000B' executed \n"
        )
    ...
    def OnStartServer(self, evt):
        if self.server_proc is not None:
            ...
            s_thread.start()
            wx.CallAfter(
                Publisher.sendMessage, "ECC_Server_Text", msg="\n\n" + openssl_cmd + "\n\n"
            )
    ...
    def OnStartClient(self, evt):
        if self.client_proc is not None:
            ...
            if self.server_proc is not None:
                ...
                wx.CallAfter(
                    Publisher.sendMessage, "ECC_Client_Text", msg="Server is not active..\n"
                )
...
class Tab_RSA_MISC(wx.Panel):
    def __init__(self, parent):
        ...
        self.command_out = wx.TextCtrl(
            self, -1, style=(wx.TE_MULTILINE | wx.TE_READONLY), size=(500, 500)
        )
        self.command_out.SetFont(
            wx.Font(12, wx.FONTFAMILY_TELETYPE, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL)
        )
        ...
        if owner_auth != "":
            ...
            if "0x81000008" in handles_output:
                ...
                self.command_out.AppendText(
                    f"'tpm2_evictcontrol -C o -P {owner_auth} -c 0x81000008' executed \n"
                )
            else:
            ...
            self.command_out.AppendText(
                f"'tpm2_createprimary -C o -P {owner_auth} -g sha256 -G ecc -c rsa_primary.ctx' executed \n"
            )
            ...
            self.command_out.AppendText(
                f"'tpm2_evictcontrol -C o -P {owner_auth} -c rsa_primary.ctx 0x81000008' executed \n"
            )
            ...
            self.command_out.AppendText(str(command_output))
            self.command_out.AppendText(
                f"'openssl pkey -provider tpm2 -provider default -in rsa2.pem -pubout -out rsa2.pub.pem' executed \n"
            )
    ...
    def OnGenKey(self):
        ...
        if owner_auth != "":
            ...
            if "0x81000009" in handles_output:
                ...
                self.command_out.AppendText(
                    f"'tpm2_evictcontrol -C o -P {owner_auth} -c 0x81000009' executed \n"
                )
            else:
            ...
            self.command_out.AppendText(
                f"'tpm2_createprimary -C o -P {owner_auth} -g sha256 -G ecc -c ecc_primary.ctx' executed \n"
            )
            ...
            self.command_out.AppendText(
                f"'tpm2_evictcontrol -C o -P {owner_auth} -c ecc_primary.ctx 0x81000009' executed \n"
            )
            ...
            self.command_out.AppendText(str(command_output))
            self.command_out.AppendText(
                f"'openssl pkey -provider tpm2 -provider default -in ecc2.pem -pubout -out ecc2.pub.pem' executed \n"
            )
...
class Tab_RNG(wx.Panel):
    def __init__(self, parent):
        ...
        self.rng_type = wx.ComboBox(
            self, -1, "RN Encoding", choices=rng_type_list, style=wx.CB_READONLY
        )
        clearimage = wx.Image("../images/clear.png", wx.BITMAP_TYPE_PNG).ConvertToBitmap()
        clearbutton = wx.BitmapButton(self, -1, clearimage)
        # ~clearbutton = wx.BitmapButton(self, -1, img.clear.getBitmap())
        ...
        # Set tooltips
        button_gen_rng.SetToolTip(
            wx.ToolTip("Generate a Random Number, output is based on the dropdown menu.")
        )
        clearbutton.SetToolTip(wx.ToolTip("Clear all textboxes."))
    ...
    def OnGenRNG(self, evt):
        ...
        if no_bytes > 128:
            no_bytes = 128
            self.command_out.AppendText(
                "Number of bytes restricted to 128 for performance purposes.\n"
            )
        ...
        if rng_output_type == 0:
            ...
            self.command_out.AppendText(
                "'openssl rand -provider tpm2 -provider default -hex "
                + str(no_bytes)
                + "' executed \n"
            )
            self.command_out.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
        # if output type is base64
            ...
            for value in split_output:
                if "warning" not in value.lower():
                    self.command_out.AppendText(value + "\n")
            self.command_out.AppendText(
                "'openssl rand -provider tpm2 -provider default -base64 "
                + str(no_bytes)
                + "' executed \n"
            )
            self.command_out.AppendText("++++++++++++++++++++++++++++++++++++++++++++\n")
