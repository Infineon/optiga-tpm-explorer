...
int main(int argc, char** argv) {
	...
	connectParams.KeepAliveInterval_sec = 10; //10
	...
	connectParams.mqttCommandTimeout_ms = 5000; //2000
	connectParams.tlsHandshakeTimeout_ms = 5000; //5000;
	...
	pulsioximeterInit(); // Init the Pulsioximeter
	...
	while ((NETWORK_ATTEMPTING_RECONNECT == rc || RECONNECT_SUCCESSFUL == rc || NONE_ERROR == rc)
		...
		INFO("-->sleep");
		sleep(1);
		sprintf(cPayload, "%s: %d, %s: %d", "SPO2", getOxygenSaturation(), "BPM", getBPM());
		clearBPM_SPO2();
