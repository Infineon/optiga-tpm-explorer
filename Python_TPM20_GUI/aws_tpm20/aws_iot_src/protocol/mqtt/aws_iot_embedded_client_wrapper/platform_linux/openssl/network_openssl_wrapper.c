...
#include <openssl/engine.h>  //openssl engine support
...
int iot_tls_connect(Network *pNetwork, TLSConnectParams params) {
	...
	//TPM Engine Support
	ENGINE	*e;
	const char 	*engine_name = "tpm2tss";
	EVP_PKEY	*pkey;
	UI_METHOD	*ui_method;
	char  *owner_auth="owner123";
	char  *parent_auth=NULL;
	//unsigned char	keyhandler[100];
	//unsigned char	*strptr;
	unsigned int	i;

	DEBUG("\n");
	DEBUG("Create-TCPSocket.");
	...
	if (!e){
		DEBUG("Load tpm2tss Engine Fail!!!");
	} else {
		DEBUG("Engine ID : %s",ENGINE_get_id(e));

		if (!ENGINE_init(e))
			DEBUG("Cannot Init TPM2 Engine!!");

		if (!ENGINE_set_default(e, ENGINE_METHOD_ALL))
			DEBUG("Cannot use TPM2 Engine!!");

		//ENGINE_ctrl_cmd(e,"PIN",0,"",NULL,0); // Remove the password request

		if (!ENGINE_ctrl(e, TPM2TSS_SET_OWNERAUTH, 0, owner_auth, NULL)) {
			DEBUG("Could not set ownerauth\n");
			return 1;
		}

		if (!ENGINE_ctrl(e, TPM2TSS_SET_PARENTAUTH, 0, parent_auth, NULL)) {
			DEBUG("Could not set parentauth\n");
			return 1;
		}
		
		

		DEBUG("Load TPM2 Device Certificate");
		...
		DEBUG("Load TPM2 Generated Key");
		ui_method = UI_OpenSSL();
		pkey = ENGINE_load_private_key(e,params.pDevicePrivateKeyLocation,ui_method,NULL);
		SSL_CTX_use_PrivateKey(pSSLContext,pkey);
		
		/*
		//pkey = ENGINE_load_private_key(e,params.pDevicePrivateKeyLocation,ui_method,NULL);
		if ((strptr = strchr(params.pDevicePrivateKeyLocation,';')) != NULL)
		{
			strcpy(keyhandler,(strptr-10));
			pkey = ENGINE_load_private_key(e,params.pDevicePrivateKeyLocation,ui_method,NULL);
			//pkey = ENGINE_load_private_key(e,keyhandler,ui_method,NULL);
			SSL_CTX_use_PrivateKey(pSSLContext,pkey);
		} else
		{
			ERROR("Load TPM2 Key fail!!");
		}
		*/

		DEBUG("Check Private Key");
		if (!SSL_CTX_check_private_key(pSSLContext))
			DEBUG("Private Key Check failed!!");
	
	}

/*	
	DEBUG("Load Device Key");
...
*/
	...
	if(ret_val == NONE_ERROR){
		...
		DEBUG("setSocketToNonBlocking..Done.");
	}

	if(NONE_ERROR == ret_val){
		ret_val = ConnectOrTimeoutOrExitOnError(pSSLHandle, params.timeout_ms);
		ret_val = SSL_get_verify_result(pSSLHandle);
		if(X509_V_OK != ret_val){
			ERROR(" Server Certificate Verification failed : %d", ret_val );
			...
			if(NULL == SSL_get_peer_certificate(pSSLHandle)){
				ERROR(" No certificate exchange happened : %d", ret_val);
				ret_val = SSL_CONNECT_ERROR;
			}
		}
		DEBUG("SSL_get_verify_result, handle : %x",pSSLHandle);
	}
	DEBUG("Return %x", ret_val);
	return ret_val;
}
...
IoT_Error_t WriteOrTimeoutOrExitOnError(SSL *pSSL, unsigned char *msg, int totalLen, int timeout_ms){
	...
	do{
		...
		// if the SSL engine requires to read, continuously call SSL_write until the SSL can finally write
		// connection is non-blocking anyway
